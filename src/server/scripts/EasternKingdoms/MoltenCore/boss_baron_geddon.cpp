...
public:
    ...
    struct boss_baron_geddonAI : public ScriptedAI
    {
        ...
        if (m_pInstance)
            m_pInstance->SetData(DATA_GEDDONISDEAD,NOT_STARTED);
        }

        void EnterCombat(Unit * /*who*/)
        {
        if (m_pInstance)
            m_pInstance->SetData(DATA_GEDDONISDEAD,IN_PROGRESS);
    }

    void JustDied(Unit *who)
    {
        if (m_pInstance)
            m_pInstance->SetData(DATA_GEDDONISDEAD,DONE);
        }
